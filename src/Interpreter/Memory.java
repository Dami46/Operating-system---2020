...
public class Memory {
    ...
    public static void readPipeFrameOdczyt(int d){
        Vector<Character> odczyt = new Vector<Character>();
        try {
            for (int i = 5; i < d+5; i++){ //5+d
                if (readPipeFrame().elementAt(0).equals('\u0000')) {
                    VirtualMemory.saveString(Shell.getPM().init, 12, "0");
                }
                else {
                    writePipe(Memory[475+i], i);
                    odczyt.add(readPipe(i));
                }
            }
